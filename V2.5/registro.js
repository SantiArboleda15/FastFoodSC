...
e-editor.js
// Función para abrir/cerrar el modal de ayuda con animación suave
...
// Función para desplegar las opciones del acordeón de ayuda
function toggleAccordion(faqId) {
    const element = document.getElementById(faqId);
    const btnSpan = element.previousElementSibling.querySelector('span:last-child');
    
    if (element.classList.contains('hidden')) {
        element.classList.remove('hidden');
        btnSpan.textContent = '−'; // Cambia el símbolo + por −
        btnSpan.classList.add('text-red-400');
    } else {
        element.classList.add('hidden');
        btnSpan.textContent = '+';
        btnSpan.classList.remove('text-red-400');
    }
}
// ── COCINA ──────────────────────────────────
if (document.getElementById('listaPedidos')) {
